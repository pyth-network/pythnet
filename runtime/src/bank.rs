...
impl Bank {
    ...
    pub fn new_with_paths(
        ...
        // If the accumulator is not moved to end of block, update the accumulator last to make sure that the solana
        // bank is in a fully updated state before the accumulator is used.
        if !bank
            .feature_set
    ...
    fn _new_from_parent(
        ...
        let (_, update_accumulator_time) = Measure::this(
            |_| {
                // If the accumulator is not moved to end of block, update the accumulator last to make sure that all
                // sysvars are in a fully updated state before the accumulator sysvar updates.
                if !new
                    .feature_set
                    .is_active(&feature_set::move_accumulator_to_end_of_block::id())
                {
